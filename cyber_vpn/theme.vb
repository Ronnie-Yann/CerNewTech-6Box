...
Class ChromeThemeContainer
        ...
        ElseIf New Rectangle(Width - 22, 5, 15, 15).Contains(New Point(X, Y)) Then
            If FindForm.ToString.Contains("Form1") Then
                If Form1.connected_yes = 0 Then
                    ...
                    End If
                    If File.Exists(Application.StartupPath & "\data\logfile.tmp") Then
                        If Form1.ConnectButton.Text = "连接中" Then
                            FindForm.Close()
                            Exit Sub
                        End If
                        Kill(Application.StartupPath & "\data\logfile.tmp")
                    End If
                    FindForm.Close()
                Else
                    Try
                        If MsgBox("想要停止" + Form1.AppName + "与服务器的连接并退出程序吗?", MsgBoxStyle.YesNo, Form1.AppName) = vbYes Then
                            g = "taskkill /f /im openvpn.exe"
                            Shell("cmd /c" & g, vbHide)
                            ...
                            Form1.InfoBar.Text = "未连接"
                            Form1.InfoBar.Update()
                            Form1.InfoBar.Refresh()
                            Form1.ConnectButton.Text = "连接"
                            Form1.ConnectButton.Refresh()
                            Kill(Application.StartupPath & "\data\*.crt")
                            Form1.NotifyIcon1.ShowBalloonTip(3000, Form1.AppName, "网络服务已停止,程序退出", ToolTipIcon.Info)
                            Form1.Close()
                        End If
    ...
    Protected Overrides Sub PaintHook()
        ...
        If New Rectangle(Width - 22, 5, 15, 15).Contains(New Point(X, Y)) Then
            G.FillEllipse(New SolidBrush(Color.FromArgb(0, 173, 220)), New Rectangle(Width - 24, 6, 16, 16))
        ...
        If New Rectangle(Width - 44, 5, 15, 15).Contains(New Point(X, Y)) Then
            G.FillEllipse(New SolidBrush(Color.FromArgb(0, 173, 220)), New Rectangle(Width - 44, 6, 16, 16))
        ...
        G.DrawLine(New Pen(Color.FromArgb(0, 173, 220), 3), New Point(0, Height), New Point(Width, Height))
